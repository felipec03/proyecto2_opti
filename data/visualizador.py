...
# Rutas absolutas para que el script funcione desde cualquier directorio
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
RESULTADOS_PATH = os.path.join(SCRIPT_DIR, "resultados.json")
MAPAS_DIR = os.path.join(SCRIPT_DIR, "mapas_resultados")
...
def cargar_resultados(ruta_archivo=None):
    if ruta_archivo is None:
        ruta_archivo = RESULTADOS_PATH
    try:
...
def generar_mapa(instancia, indice, carpeta_salida):
    ...
    nombre_limpio = (
        nombre_instancia.replace(":", "").replace(" ", "_").replace("–", "-").lower()
    )
...
def generar_todos_los_mapas():
    ...
    print("-" * 50)
    print(
        f"¡Proceso completado! Se han guardado {len(resultados)} mapas en la carpeta '{carpeta_salida}'.\n"
    )
