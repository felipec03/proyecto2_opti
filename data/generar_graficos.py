...
def grafico_comparacion_fobj(resultados, instancias):
    ...
    bars = ax.barh(y_pos, valores, color=colores, edgecolor="white", height=0.65, zorder=3)

    # Línea de referencia del escenario base
    ax.axvline(base_val, color=COLOR_OBJETIVO, linestyle="--", linewidth=1.5,
               alpha=0.7, zorder=2, label=f"Escenario Base: {base_val:,.0f} CLP/día")
    ...
    ax.set_title("Comparación del Costo Total Óptimo por Escenario\n"
...
def grafico_desglose_costos(resultados, instancias):
    ...
    bars_fijo = ax.bar(x, costos_fijos, width, label="Costo Fijo (CAPEX+OPEX)",
                       color=COLOR_FIJO, edgecolor="white", zorder=3)
    bars_transp = ax.bar(x, costos_transp, width, bottom=costos_fijos,
                         label="Costo Transporte",
                         color=COLOR_TRANSPORTE, edgecolor="white", zorder=3)

    # Porcentaje de transporte encima
    for i, (cf, ct) in enumerate(zip(costos_fijos, costos_transp)):
        total = cf + ct
        pct_t = (ct / total) * 100
        ax.text(i, total + 50000, f"{pct_t:.0f}% transp.",
    ...
    ax.set_title("Desglose del Costo Total: Fijo vs. Transporte\n"
    ...
    fig.text(0.5, -0.02, legend_text, ha="center", fontsize=7, color="#666666",
             style="italic")
...
def grafico_heatmap_plantas(resultados, instancias):
    ...
    ax.set_xticklabels([f"I{i+1}" for i in range(n_inst)], fontsize=10, fontweight="bold")
    ax.set_yticks(np.arange(n_plantas))
    ax.set_yticklabels([f"J{j+1}: {PLANTAS[j+1]}" for j in range(n_plantas)], fontsize=11)
    ...
    for i in range(n_plantas):
        for j_idx in range(n_inst):
            ...
            ax.text(j_idx, i, texto, ha="center", va="center",
                    fontsize=8, color=color, fontweight=weight)

    ax.set_title("Decisiones de Apertura de Plantas (Y_j) por Escenario\n"
...
def grafico_flujos_asignacion(resultados, instancias, idx_highlight=[0, 1, 9]):
    ...
    for idx in idx_highlight:
        ...
        for i in range(1, n_ms + 1):
            color_ms = "#457B9D"
            ax.annotate(f"  {MACROSECTORES[i]}", xy=(0.12, y_ms[i-1]),
        ...
        for j in [1, 2, 3]:
            ...
            estado = "✓" if abierta else "✗"
            ax.annotate(f"{PLANTAS[j]} {estado}  ", xy=(0.88, y_pl[j]),
        ...
        for clave, cantidad in flujos.items():
            ...
            ax.annotate("", xy=(0.88, y_pl[j]), xytext=(0.12, y_ms[i-1]),
                        arrowprops=dict(arrowstyle="->,head_width=0.15",
                                        color=color_linea, lw=ancho, alpha=alpha))
            # Etiqueta de flujo
            mid_x = 0.5
            mid_y = (y_ms[i-1] + y_pl[j]) / 2
            ax.text(mid_x, mid_y, f"{cantidad:.0f}t",
                    fontsize=7, ha="center", va="center",
                    color=color_linea, fontweight="bold", alpha=0.85,
                    bbox=dict(boxstyle="round,pad=0.15", facecolor="white",
                              edgecolor=color_linea, alpha=0.7))
        ...
        ax.set_title(f"Flujos de Asignación X_ij [ton/día] – I{idx+1}: {titulo}\n"
                     f"F. Obj. = ${fobj:,.0f} CLP/día",
                     fontsize=14, pad=15, fontweight="bold")

        # Leyenda
        ax.text(0.05, 0.02, "● Macrosectores generadores",
                fontsize=8, color="#457B9D", transform=ax.transAxes)
        ax.text(0.55, 0.02, "■ Plantas de valorización candidatas",
                fontsize=8, color="#E63946", transform=ax.transAxes)
...
def grafico_demanda_macrosectores(resultados, instancias):
    ...
    for k, sel_idx in enumerate(instancias_selec):
        ...
        ax.bar(x + offset + k * width, demandas, width,
               label=nombre, color=colores_sel[k], edgecolor="white", zorder=3)

    ax.set_xticks(x)
    ax.set_xticklabels([MACROSECTORES[i] for i in range(1, 9)],
                       rotation=25, ha="right", fontsize=9)
    ax.set_ylabel("Demanda W_i [ton/día]", fontweight="bold")
    ax.set_title("Variación de la Demanda por Macrosector entre Escenarios\n"
...
def grafico_utilizacion_capacidad(resultados, instancias):
    ...
    for idx, (r, inst) in enumerate(zip(resultados, instancias)):
        ...
        for k, (fl, cap, pct) in enumerate(zip(flujos_arr, caps, pcts)):
            if fl > 0:
                ax.text(k, fl + 5, f"{pct:.0f}%", ha="center", fontsize=8,
    ...
    fig.suptitle("Utilización de Capacidad de Plantas – 10 Escenarios\n"
...
def grafico_tiempos_computacionales(resultados, instancias):
    ...
    colores = ["#2A9D8F" if t < 20 else "#E9C46A" if t < 40 else "#E63946"
               for t in tiempos_ms]

    bars = ax.bar(nombres, tiempos_ms, color=colores, edgecolor="white",
                  width=0.6, zorder=3)
    ...
    ax.set_title("Tiempo Computacional por Instancia – Solver CBC (PuLP)\n"
                 "El paper usa Gurobi (MINLP regional); nuestro MILP comunal es rápido",
                 fontsize=14, pad=15)
    ax.grid(axis="y", alpha=0.3, color=COLOR_GRID, zorder=0)

    # Referencia al paper
    ax.text(0.98, 0.95, "Paper (López Marín, 2024): MINLP regional\n"
...
def grafico_contraste_paper(resultados, instancias):
    ...
    ax.barh(y + height/2, [8, 3, 3, 0], height, label="Nuestro modelo (Temuco)",
            color="#2A9D8F", edgecolor="white", zorder=3)
    ax.barh(y - height/2, [32, 6, 70, 0], height, label="Paper (Araucanía completa)",
            color="#E76F51", edgecolor="white", zorder=3)

    for i, (tn, tp) in enumerate(zip(tipo_nuestro, tipo_paper)):
        if i < 3:
            ax.text(max(nuestro[i] if nuestro[i] else 0, 2) + 1, i + height/2,
                    tn, va="center", fontsize=10, fontweight="bold", color="#2A9D8F")
            ax.text(max(paper[i] if paper[i] else 0, 2) + 1, i - height/2,
                    tp, va="center", fontsize=10, fontweight="bold", color="#E76F51")
        else:
            ax.text(5, i + height/2, tn, va="center", fontsize=12, fontweight="bold",
                    color="#2A9D8F")
            ax.text(5, i - height/2, tp, va="center", fontsize=12, fontweight="bold",
                    color="#E76F51")
    ...
    fig.suptitle("Contraste: Nuestro Modelo MILP Comunal vs. Paper MINLP Regional\n"
...
def grafico_sensibilidad(resultados, instancias):
    ...
    # Graficar
    ax.plot(angulos, valores_norm, "o-", color="#E63946", linewidth=2,
            markersize=8, zorder=5)
    ax.fill(angulos, valores_norm, color="#E63946", alpha=0.15)

    # Línea base
    ax.plot(angulos, [1.0] * len(angulos), "--", color="#2A9D8F",
            linewidth=2, alpha=0.7, label="Escenario Base (ratio = 1.0)")

    # Etiquetas
    etiquetas = [f"I{i+2}: {nombre_corto(r['nombre'])}"
    ...
    for ang, val, r in zip(angulos[:-1], valores_norm[:-1], resultados[1:]):
        pct = (val - 1) * 100
        signo = "+" if pct > 0 else ""
        ax.annotate(f"{signo}{pct:.1f}%",
                    xy=(ang, val), xytext=(5, 12),
                    textcoords="offset points", fontsize=8,
                    fontweight="bold", color="#333", ha="center")

    ax.set_title("Análisis de Sensibilidad – Desviación Relativa al Escenario Base\n"
...
def grafico_tabla_resumen(resultados, instancias):
    ...
    columnas = ["Instancia", "Escenario", "F. Obj.\n[CLP/día]",
                "Costo Fijo\n[CLP/día]", "Costo Transp.\n[CLP/día]",
                "Plantas\nAbiertas", "Tiempo\n[ms]", "Δ vs Base"]
    ...
    table = ax.table(cellText=datos, colLabels=columnas,
    ...
    ax.set_title("Resumen Ejecutivo – Resultados de las 10 Instancias CFLP\n"
